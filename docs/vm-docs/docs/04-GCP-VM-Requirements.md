...
Thank you for choosing to be­come a part­ner for the eCR View­er! This doc­u­ment is de­signed to help you pre­pare for your first GCP de­ploy­ment of the eCR View­er VM.
...
You can lever­age ei­ther the Google Cloud Platform Dashboard or the GCP CLI tool to per­form most of the operations in this guide. We will pro­vide spe­cif­ic CLI com­mands along with links to official GCP docs where ap­pro­pri­ate.
...
- You’ll need to have an account, per­mis­sions, and nec­es­sary func­tion­al­i­ty set up in GCP. 
- You’ll need to be able to setup a Storage bucket in your GCP account to store your eCR data, with the prop­er net­work­ing and ac­cess pro­vi­sions for your VM.
- You’ll need to have a data­base set up (on-premise or cloud-based, Mi­crosoft SQL or  Post­gre­SQL) and ac­ces­si­ble by the GCP net­work. 
- You’ll need to have an Entra, or Key­cloak client created or NBS authentication for the eCR View­er ap­pli­ca­tion.
- You’ll need to have IAM permissions to make adjustments within your GCP en­vi­ron­ment.
...
- Must be reach­able over the net­work, ei­ther with­in your GCP account, via net­work peering, or via a net­work route to your on premise in­stance. 
...
Your vir­tu­al ma­chine instance will need permissions and network access to the Cloud Storage Bucket, the Database (if applicable), and Entra or Keycloak (if applicable) to function correctly.
...
Once you've setup your VM, vis­it http://__VM_AD­DRESS__:9000 in your brows­er of choice. It is recommended that you restrict access to Portainer using security groups, allowing only the administrator IP access when needed. Click here for official docs re­gard­ing Por­tain­er’s func­tions.
...
- Ac­ti­vate a fire­wall with­in GCP.
