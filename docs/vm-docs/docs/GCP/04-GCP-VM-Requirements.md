...
Thank you for choosing to be­come a part­ner for the eCR View­er! This document is de­signed to help you pre­pare for your first GCP deployment of the eCR View­er VM.
...
You can lever­age either the Google Cloud Platform Dashboard or the GCP CLI tool to per­form most of the operations in these guides.
...
- You’ll need to have an account, per­missions, and necessary function­al­i­ty set up in GCP. 
- You’ll need to be able to setup a Storage bucket in your GCP account to store your eCR data, with the prop­er net­work­ing and ac­cess pro­visions for your VM.
- You’ll need to have a data­base set up (on-premise or cloud-based, Microsoft SQL or  Post­gre­SQL) and ac­ces­si­ble by the GCP net­work. 
...
- Must be reach­able over the net­work, either with­in your GCP account, via net­work peering, or via a net­work route to your on premise in­stance. 
...
Once you've setup your VM, vis­it http://__VM_AD­DRESS__:9000 in your brows­er of choice. It is recommended that you restrict access to Portainer using security groups, allowing only the administrator IP port access when needed. 
...
- Activate a fire­wall with­in GCP.
